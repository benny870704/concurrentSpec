from example_3_scenario_outline.src.shopping_cart import ShoppingCart
...
class RemoveItemFromCart:
    ...
    def when_i_remove_some_items_from_my_cart(self, some):
        items = self.shopping_cart.get_items()
        removed_number = int(some)
        while removed_number > 0:
            self.shopping_cart.remove_item(items[removed_number-1])
            removed_number -= 1

    def then_i_should_see_remaining_items_in_my_cart(self, remaining):
        assert int(remaining) == self.shopping_cart.get_items_count()
